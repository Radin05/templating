                    ...
                    <form action="{{ route('user.update', $user->id) }}" method="POST" role="form" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <select type="" class="form-control  @error('is_Admin') is-invalid @enderror" name="is_admin"
                                placeholder="Role" value=' {{ $user->is_admin }}' required>
                                <option value="1" {{ $user->is_admin ? 'selected'  : '' }}>admin</option>
                                <option value="0" {{ $user->is_admin == 0?  'selected' : ''}}>user</option>
                            </select>
                        </div>
                        <a href="{{route('user.index')}}" class="mt-4 btn-secondary btn btn-default">Kembali</a>
                        <button type="submit" class="mt-4 btn-primary btn btn-default">Tambah</button>
                    </form>
                </div>
            </div>
        </div>
